...
//#region src/config.util.ts
const loadConfig = (configPath) => {
	const resolvedPath = path.resolve(configPath || process.cwd(), "commit-config.json");
	if (!fs.existsSync(resolvedPath)) throw new Error("❌ commit-config.json not found in current directory");
	const data = fs.readFileSync(resolvedPath, "utf8");
	const { language, conventions, agentConfig, models, isPipeline, pipelineConfig, isAutoPush } = JSON.parse(data);
	if (typeof language !== "string") throw new Error("❌ Invalid config: 'language' must be a string.");
	if (!Array.isArray(conventions)) throw new Error("❌ Invalid config: 'conventions' must be an array.");
	if (!Array.isArray(models) || models.length === 0) throw new Error("❌ Invalid config: 'models' must be a non-empty array.");
	if (isPipeline && !pipelineConfig) throw new Error("❌ Invalid config: 'pipelineConfig' must be provided when 'isPipeline' is true.");
	return {
		commitConfig: {
			language,
			conventions
...
//#endregion
//#region src/core/git.util.ts
const getGitDiff = (mode, repoPath) => {
	try {
		let command = "";
		switch (mode) {
			case "staged":
		...
		if (!diff.trim()) console.log("⚠️  No changes detected for the selected mode.");
		return diff;
	} catch (err) {
...
function validateDiffSize(diff, maxChars = 5e4) {
	if (diff.length > maxChars) throw new Error(`❌ Diff too large (${(diff.length / 1e3).toFixed(1)} KB).\nAllowed max ${maxChars} characters.\nTip: Stage fewer files or commit incrementally.`);
}
function commitChanges({ commitMessage, diffMode, execOptions }) {
	try {
		if (diffMode === "all") {
			console.log("📦 Staging all files...");
			execSync("git add .", execOptions);
		}
		execSync(`git commit -m "${commitMessage.replace(/"/g, "\\\"")}"`, execOptions);
		console.log("✅ Changes committed successfully.");
	} catch (err) {
		console.error("❌ Error committing changes:", err);
	}
}
function pushChanges(execOptions) {
	try {
...
function gitPostJob({ isAutoPush, commitMessage, diffMode, execOptions }) {
...
//#endregion
//#region src/core/prompt.util.ts
const getSystemPrompt = ({ numberOfSuggestions, language, conventions }) => `
...
`;
const getUserPrompt = ({ diff, language }) => `
...
`;
const getConventionString = (conventions) => {
	return conventions.map((c) => `${c.prefix}: ${c.description}`).join("; ");
};

//#endregion
//#region src/cli/models/schema.ts
const CommitMessageSchema = z.object({ commitMessages: z.array(z.string()).min(1) });

//#endregion
//#region src/cli/models/openAi.ts
async function getOpenAiCommitMessage(params) {
	try {
		...
		const content = (await new OpenAI({ apiKey }).chat.completions.create({
			model: model || "gpt-4o",
			messages: [{
				role: "system",
...
//#endregion
//#region src/cli/models/gemini.ts
async function getGeminiCommitMessage(params) {
	const { diff, commitConfig: { conventions, language }, agentConfig: { temperature, maxTokens }, model: { apiKey, model }, numberOfSuggestions = 3 } = params;
...
//#endregion
//#region src/cli/models/getCommitMessageFactory.ts
const getCommitMessageFactory = async (params) => {
	const { model: { type } } = params;
	switch (type) {
		case AIModelEnum.OPENAI: return getOpenAiCommitMessage(params);
		case AIModelEnum.GEMINI: return getGeminiCommitMessage(params);
		default: throw new Error(`❌ Unsupported model type: ${type}`);
	}
};

//#endregion
//#region src/cli/userRun.ts
const userRun = async (config) => {
	const { diffMode } = await inquirer.prompt([{
		...
		choices: [{
			...
			name: "All changes (working tree)",
			value: "all"
		}],
		default: "staged"
	}]);
	const diff = getGitDiff(diffMode);
...
//#endregion
//#region src/cli/pipelineRun.ts
const pipelineRun = async (config) => {
	const { commitConfig, agentConfig, models, isAutoPush, pipelineConfig: { modelIndex = 0, diffMode = "staged" } = {} } = config;
	const diff = getGitDiff(diffMode);
	validateDiffSize(diff);
	let model = void 0;
	if (models.length === 1) model = models[0];
	else model = models[modelIndex];
	console.log(`- Using model: ${model.type}`);
	if (!model) {
		console.error("❌ No model selected.");
		return;
	}
	const selectedCommit = (await getCommitMessageFactory({
		diff,
		commitConfig,
		agentConfig,
		model,
...
const CommitChangesInputSchema = z.object({
	repoPath: z.string(),
...
async function mcpRun() {
	...
	server.registerTool("generate_ai_commits", {
		title: "Generate Commit Messages",
		description: "Generates commit messages from git diff and given conventions.",
		inputSchema: GenerateCommitInputSchema
	}, async (args) => {
		try {
			const { repoPath, diffMode = "staged", numberOfSuggestions = 3 } = GenerateCommitInputSchema.parse(args);
			const { commitConfig: { language, conventions } } = loadConfig(repoPath);
			const diff = getGitDiff(diffMode, repoPath);
			if (!diff.trim()) return { content: [{
				type: "text",
				text: "No git diff found."
			}] };
			return { content: [{
				type: "text",
				text: `
...
${getUserPrompt({
	...
	server.registerTool("commit_generated_message", {
		title: "Commit Generated Message",
		description: "Commits the generated commit message to the git repository.",
		inputSchema: CommitChangesInputSchema
	}, async (args) => {
		try {
			const { repoPath, message, diffMode, isAutoPush } = CommitChangesInputSchema.parse(args);
			gitPostJob({
				isAutoPush,
...
const main = async () => {
	try {
		const cliMode = process.argv.slice(2)[0];
		console.error(`CLI Mode: ${cliMode}`);
		if (cliMode === "--mcp") {
			await mcpRun();
			return;
		}
